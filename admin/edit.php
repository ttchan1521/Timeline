...
<html>
    ...
    <body>
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'edit') {
            if (isset($_POST['userID']) && !empty($_POST['userID']) && isset($_POST['old_password']) && !empty($_POST['old_password']) && isset($_POST['new_password']) && !empty($_POST['new_password'])
            ) {
                $userResult = mysqli_query($con, "Select * from `users` WHERE (`userID` = " . $_POST['userID'] . " AND `accountPassword` = '" . md5($_POST['old_password']) . "')");
                if ($userResult->num_rows > 0) {
                    $result = mysqli_query($con, "UPDATE `users` SET `accountPassword` = MD5('" . $_POST['new_password'] . "'), `last_updated`=" . time() . " WHERE (`userID` = " . $_POST['userID'] . " AND `accountPassword` = '" . md5($_POST['old_password']) . "')");
                    if (!$result) {
            ...
            if (!empty($user)) {
                ...
                <div id="edit_user" class="box-content">
                    <h1>Xin chào "<?= $user['firstName'] ?>". Bạn đang thay đổi mật khẩu</h1>
